...
In this post, I will catch up on the options taken-in scripting in [Shell](#Shell), [R](#R) and [Perl](#Perl).

### Shell {#Shell}

Shell script takes in first type of options using special variables based on the input orders `$1`, `$2`, ... For the unknown number of inputs, `$@` array is used to represents all arguments after script file (`$0`).
